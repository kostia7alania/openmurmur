...
| --- | --- | --- |
| Offline automated | On 2026-09-07, typecheck, Biome, all 720/720 Node tests in a sequential resource-stable run, and 39/39 Python pytest cases passed. | No microphone, model weights, Telegram credential, Keychain mutation, launchd login session or sleep/wake cycle is exercised. |
...
| Current host follow-up | On 2026-09-07 the updated daemon restarted with a fresh heartbeat and zero processing lag; two acknowledged recording-status updates retained the same Telegram message ID across restart. The live silence/stale-input timeouts are 60 seconds. The capture diagnostic now rejects all-zero PCM with exit 1, verified by a 158-frame native probe and 26/26 targeted diagnostic/setup/CLI tests. | Microphone acceptance is blocked: authorization is granted and the lid is open, but the native probe contains only digital silence; the system input meter remains stationary after an audio-service restart. Expandable notification quotes have automated transport coverage, not a live rendering sign-off. |
