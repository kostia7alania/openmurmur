...
| Current host follow-up | On 2026-09-07 the updated daemon restarted with a fresh heartbeat and zero processing lag; two acknowledged recording-status updates retained the same Telegram message ID across restart. The live silence/stale-input timeouts are 60 seconds. | Microphone acceptance is blocked: authorization is granted and the lid is open, but a 156-frame native probe contains only digital silence; the system input meter remains stationary after an audio-service restart. Expandable notification quotes have automated transport coverage, not a live rendering sign-off. |
