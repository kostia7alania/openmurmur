...
- [ ] **PX-42 · P0 · gap** — Guard ASR Auto against unsupported language drift. Evidence: real incoming `#job9` died after Qwen Auto entered Japanese tokenization and `nagisa` was missing, while the product language set is RU/TH/EN/ZH. Accept: Auto choosing an unsupported language is classified as an actionable ASR failure, one stray foreign token does not promote the whole job to a new product language, and a selected retry can force `ru`, `th`, `en` or `zh` without reusing the stale Auto failure. See `docs/BACKLOG.md` UX-00.
