...
- [ ] **CS-10 · P1 · gap** — Fence job completion/failure by lease owner and expected state. Evidence: `src/jobs/queue.ts:146-183` updates by job id. Accept: an expired worker cannot complete or fail a job reclaimed by another worker; smallest test uses two owners.
- [ ] **CS-30 · P1 · verify-live** — Prove or replace the fixed lease for long-running handlers. Evidence: queue lease is 10 minutes while ASR timeout can be 15 minutes. Accept: heartbeat/per-kind lease prevents concurrent claims under a deliberately slow handler.
- [ ] **RO-04 · P0 · confirmed** — Add lease heartbeats or per-kind leases for ASR. Evidence: `src/jobs/queue.ts:93`, `src/config/schema.ts:211` and global reclaim in `src/cli/daemon.ts:537-540`. Accept: a 15-minute ASR cannot be double-claimed after the 10-minute boundary.
...
- [ ] **CS-11 · P1 · gap** — Fence outbox terminal updates by expected state/owner. Evidence: `src/telegram/outbox.ts:117-157`. Accept: a stale sender cannot mark a reclaimed delivery sent/dead; smallest test races two owners.
...
- [ ] **CS-34 · P1 · confirmed** — Bound `health_events` growth. Evidence: `src/cli/daemon.ts:1032-1039` and `src/health/monitor.ts:169-173` insert every unhealthy poll. Accept: edge/bucket sampling plus retention keeps diagnostic value with bounded rows.
- [ ] **CS-35 · P1 · confirmed** — Treat disk/SQLite probe errors as unknown/unhealthy. Evidence: `diskFreeGb` returns `Infinity` and `sqliteWritable` performs only a read. Accept: statfs failure never reports healthy and a rollback-only write probe detects read-only storage.
- [ ] **RO-13 · P1 · confirmed** — Store health transitions or bounded samples, not every bad poll. Evidence: `src/cli/daemon.ts:1036-1043` and `src/health/monitor.ts:169-172`. Accept: a day-long outage has bounded rows plus raised/recovered timestamps.
- [ ] **RO-14 · P1 · confirmed** — Represent statfs failure explicitly. Evidence: `src/health/monitor.ts:150-156` maps it to infinite free space. Accept: `/health` and alerts show unknown/failure, never green.
- [ ] **RO-15 · P1 · confirmed** — Probe SQLite writability with a rolled-back write. Evidence: `src/health/monitor.ts:159-166` runs only `SELECT`. Accept: read-only/open-but-unwritable database fails the check without persistent mutation.
