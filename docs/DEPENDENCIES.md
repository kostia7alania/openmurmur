...
All versions below were refreshed on the development machine on **2026-09-01**
(macOS 26.5.1, Apple Silicon), unless the row says otherwise. Anything not
...
| --- | --- | --- | --- |
| Node.js | 26.8.1 | ✅ executed | Current, **not yet LTS** — see [ADR-0001](adr/0001-node-26.md). |
| TypeScript | 7.0.2 | ✅ typecheck passed | Latest stable; ships as `tsc`, not `tsgo`. |
| pnpm | 11.25.0 | ✅ frozen install succeeded | The registry `latest` dist-tag on 2026-09-01. |
| Biome | 2.5.11 | ✅ 132 files checked | Replaces ESLint + Prettier — see [ADR-0005](adr/0005-biome-over-eslint.md). |
| `@types/node` | 26.4.0 | ✅ installed | Latest published. |
| SQLite (`node:sqlite`) | **3.53.4** | ✅ queried at runtime | Compiled into Node 26.8.1 — see [ADR-0004](adr/0004-sqlite-driver.md). |
| FFmpeg / ffprobe | 9.0.1 | ✅ Node integration gate passed | `brew install ffmpeg`. |
| Python | 3.14.7 | ✅ 39 pytest cases passed | Current project interpreter with wheels across the whole stack. |
| uv | 0.12.7 | ✅ `uv sync --extra mlx` succeeded | |
...
### Latest-compatible refresh — 2026-09-01

The repository pin moved to Node 26.8.1 and pnpm 11.25.0. The Node lockfile was
regenerated by that exact manager, and the Python lock was fully upgraded by
uv 0.12.7 against CPython 3.14.7. The refresh keeps existing major-version and
Python-minor boundaries: TypeScript stays on its latest stable 7.0.2 release,
and Python 3.15 remains excluded until the ML stack publishes compatible wheels.

Typecheck, Biome, all 714 Node tests in a sequential resource-stable run, 39
Python tests, Ruff lint and format, and strict mypy all passed on the refreshed
environment. The real
Ollama model canary remains pending while the new 18 GB `qwen3.8:27b` model downloads;
no pending live gate is represented as passed.
...
| --- | --- | --- | --- |
| numpy | 2.5.2 | ✅ installed | Array handling. Requires ≥ 3.12. |
| onnxruntime | 1.29.0 | ✅ installed | Silero VAD inference. **Publishes cp311–cp314 only**, which is what caps Python at 3.14. |
| soundfile | 0.14.0 | ✅ imported | Audio file reading. |
| pytest | 9.1.1 | ✅ 39 passed | |
| ruff | 0.16.5 | ✅ lint and format checks passed | Lint and format. |
| mypy | 2.3.1 | ✅ 7 source files passed strict mode | Strict mode. |

### Optional `mlx` extra — installed and run on the development machine, never in CI

| Package | Pinned | Status |
| --- | --- | --- |
| mlx | ≥0.32.2,<0.33 | ✅ Installed on this Apple Silicon Mac. Wheels cp310–cp314, macOS arm64. |
...
| sherpa-onnx | ≥1.13.6,<2 | ✅ Imported with the refreshed native core. |
| sherpa-onnx-core | ≥1.13.6,<2 | ✅ Named explicitly: resolving the extra installed only the wrapper, and the native module then failed to load its bundled `libonnxruntime`. Carries its own onnxruntime; does not conflict with the pin above. |
...
| --- | --- | --- | --- |
| Ollama | 0.33.2; `qwen3.8:27b` | ⚠️ real single-language RU/EN/TH corpus executed; acceptance not met | Optional. Its absence degrades the report; it never blocks delivery. |
...
```
onnxruntime 1.29.0 → cp311, cp312, cp313, cp314   ← the binding constraint
...
developer's machine. 3.14.7 is the newest version the whole stack actually
